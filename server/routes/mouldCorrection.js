...
router.post('/', verifyToken, asyncErrorHandler(async (req, res, next) => {
    ...
    const [audit_result] = await Client.query(audit_sql, [req.user.user_id, req.user.department_id, trial_id, 'Mould correction created', `Mould correction ${trial_id} created by ${req.user.username} with trial id ${trial_id}`]);
